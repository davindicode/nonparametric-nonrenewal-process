...
def newton_cotes(f, h, order='trapezoid'):
    ...
    if order == 'trapezoid':
        ...
        coeff = coeff.at[0].set(.5).at[-1].set(.5)
...
def time_transform(t, t0, inverse=False):
    """
    Inverse transform is from tau [0, 1] to t in R
    """
    if inverse:
        t_ = -jnp.log(1 - t) * t0
        log_jac = jnp.log(t0) - jnp.log(1 - t) # t0 / (1 - t)
    else:
        s = jnp.exp(-t / t0)
        t_ = 1 - s
        log_jac = -t / t0 - jnp.log(t0)  # s / t0
    return t_, log_jac


def refractory_mean(tau, tau0, neg=-12.):
    return neg * jnp.exp(- tau / tau0)
