...
def vmap_spatial(H, minf, Pinf, As, Qs, site_obs, site_Lcov, ind_eval, A_fwd, A_bwd, mean_only, compute_KL, jitter):
    return vmap_outdims(
...
class KroneckerLTI(SSM):
    ...
    def evaluate_posterior(
        ...
        if self.spatial_MF:  # vmap over temporal kernel out_dims = spatial_locs
            ...
            H, minf, Pinf, As, Qs = self.markov_sparse_kernel._get_LDS(site_dlocs, site_locs.shape[1])
            # (ts, out_dims, sd, sd)
            ...
            stack_A = vmap(self.markov_sparse_kernel.state_transition)(stack_dt)  # vmap over num_evals
            A_fwd, A_bwd = stack_A[:num_evals], stack_A[-num_evals:]
            # (eval_inds, out_dims, spatial_pts*sd, spatial_pts*sd)
            
            H, minf, Pinf, As, Qs = self.markov_sparse_kernel.get_LDS(site_dlocs, site_locs.shape[1])
