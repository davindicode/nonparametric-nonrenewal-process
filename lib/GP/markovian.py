...
def sample_LGSSM(H, minf, P0, As, Qs, prng_state, num_samps, jitter):
    ...
    def sample_i(prng_state):
        m0 = cholesky(P0) @ jr.normal(
            prng_state, shape=(state_dims, 1)
        )
...
class LGSSM(SSM):
    ...
    def sample_prior(self, prng_state, num_samps, jitter):
        ...
        """
        H, minf, P0, As, Qs = self.get_LDS()
        return sample_LGSSM(H, minf, P0, As, Qs, prng_state, num_samps, jitter)  # (time, tr, state_dims, 1)
...
class GaussianLTI(SSM):
    ...
    def evaluate_posterior(
        ...
        stack_A = vmap(self.markov_kernel.state_transition)(stack_dt)  # vmap over num_evals
        ...
        post_means, post_covs, KL = evaluate_LTI_posterior(
            H, minf, Pinf, As, Qs, self.site_obs, self.site_Lcov, 
            ind_eval, A_fwd, A_bwd, mean_only, compute_KL, jitter, 
        )
        return post_means, post_covs, KL
    ...
    def sample_posterior(
        ...
        post_means, _, KL_ss = evaluate_LTI_posterior(
            H, minf, Pinf, As, Qs, self.site_obs, self.site_Lcov, 
            ind_eval, A_fwd, A_bwd, mean_only=True, compute_KL=compute_KL, jitter=jitter,
        )  # (time, out_dims, 1)
...
class MultiOutputLTI(GaussianLTI):
    ...
    def __init__(self, markov_kernel, site_locs, site_obs, site_Lcov, fixed_grid_locs=False):
        """
    ...
    ### posterior ###
    def evaluate_posterior(
        self, t_eval, x_eval, mean_only, compute_KL, jitter, 
    ):
        """
        predict at test locations X, which may includes training points
        (which are essentially fixed inducing points)
        ...
        ind_eval, dt_fwd, dt_bwd = vmap(interpolation_times, (0, 0), (1, 1, 1))(
            t_eval, site_locs)  # vmap over out_dims
        ...
        if self.spatial_MF:  # vmap over temporal kernel out_dims = spatial_locs
            stack_A = vmap(self.markov_kernel._state_transition)(
                stack_dt)  # vmap over num_evals, (eval_inds, out_dims, sd, sd)
            
            # compute LDS matrices
            H, minf, Pinf, As, Qs = self.markov_kernel._get_LDS(site_dlocs, site_locs.shape[1])
            # (ts, out_dims, sd, sd)
    ...
    def sample_prior(self, prng_state, t_eval, x_eval, jitter):
        ...
        # evaluation locations
        t_all, site_ind, eval_ind = vmap(order_times, (None, 0), (0, 0, 0))(t_eval, site_locs)
