...
class FactorizedGPLVM(FilterGPLVM):
    ...
    def sample_prior(self, prng_state, num_samps, x_obs=None, timedata=None):
        ...
        """
        pf_x = self.gp.sample_prior(
            prng_state, xx.repeat(num_samps, axis=0), jitter
        )  # (evals, samp, f_dim)
...
class RenewalGPLVM(FilterGPLVM):
    ...
    def sample_instantaneous_renewal(
        self, prng_state, t_eval, x_cond, num_samps=20, prior=True
    ):
        """
        :param jnp.ndarray t_eval: evaluation times since last spike, i.e. ISI
        """
        f_samp = self._gp_sample(
            prng_state, x_cond, prior, jitter
        )  # (num_samps, obs_dims)
        tau_eval = (
            self.inverse_link(f_samp) * t_eval[:, None] / self.shape_scale()[None, :]
        )
    ...
    def sample_prior(self, prng_state, num_samps, x_obs=None, timedata=None):
        ...
        spiketimes = gen_IRP(self.sample_ISI, rate[:, neuron, :], self.dt.item())
...
class NonparametricPPGPLVM(FilterGPLVM):
    ...
    def refractory_mean(self, tau, neg=-12.0):
        """
    ...
    def sample_instantaneous_renewal(
        ...
        quad_rhotau_eval = vmap(vvinterp, (1, None, None), 2)(
            locs[..., 0], tau_pts, rhotau_pts
        )
    ...
    def evaluate_conditional_intensity(self, prng_state, t_eval, x_eval):
        ...
        tau_since = self._tau_since_spike(
            self.renewal.dt, tau_start, t_since_spike, rates
        )
