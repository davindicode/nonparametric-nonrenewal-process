...
class GaussianLatentObservedSeries(module):
    ...
    def __init__(
        self, ssgp, lat_dims, obs_dims, diagonal_cov=False, array_type="float32"
    ):
    ...
    def sample_prior(self, prng_state, num_samps, timestamps, x_eval, jitter):
        ...
        if len(self.lat_dims) == 0:
            ...
            x_samples = self.ssgp.sample_prior(
                prng_state, num_samps, timestamps, jitter
            )[..., 0]
    ...
    def sample_posterior(
        ...
        if len(self.lat_dims) == 0:
            ...
            x_samples, KL = self.ssgp.sample_posterior(
                prng_state, num_samps, timestamps, jitter, compute_KL
            )  # (tr, time, x_dims)
    ...
    def marginal_posterior(self, num_samps, timestamps, x_eval, jitter, compute_KL):
        ...
        if len(self.lat_dims) == 0:
            ...
            post_mean, post_cov = post_mean[None].repeat(num_samps, axis=0), post_cov[
                None
            ].repeat(num_samps, axis=0)
    ...
    def sample_marginal_posterior(
        ...
        if self.diagonal_cov:
            ...
            x = (
                x_mean + (Lcov @ jr.normal(prng_state, shape=x_mean.shape))[..., 0]
            )  # (num_samps, ts, x_dims)
        return x, KL
