...
from ..utils.neural import time_rescale

from .base import FilterObservations, Observations, SparseGPFilterObservations
...
class ModulatedFactorized(SparseGPFilterObservations):
    ...
    def variational_expectation(
        ...
        if log_predictive:
            Eq = jax.nn.logsumexp(
                vmap(vmap(llf), (None, 0, 0, 0))(ys.T, f_mean, f_cov, prng_state).mean(
                    1
                )
                - jnp.log(num_samps),
                axis=0,
    ...
    def evaluate_pre_conditional_rate(self, prng_state, x_eval, y_filt, jitter):
        ...
        """
        pre_rates_mean, pre_rates_cov, _ = self._pre_rho_posterior(
            prng_state,
            x_eval,
            y_filt,
            mean_only=False,
    ...
    def evaluate_posterior_mean(self, prng_state, xs, ys_filt, jitter):
        """
        ...
        if self.likelihood.link_type == LinkTypes["log"]:
            log_rho_t_mean, log_rho_t_cov, _ = self._pre_rho_posterior(
                prng_state,
                xs[None, None],
                ys_filt,
                mean_only=False,
            ...
            log_rho_t_mean, log_rho_t_cov = (
                log_rho_t_mean[0, ..., 0],
                log_rho_t_cov[0, ..., 0],
            )  # (obs_dims, ts)
            post_rho_mean = jnp.exp(log_rho_t_mean + log_rho_t_cov / 2.0)
        else:
    ...
    def sample_prior(self, prng_state, num_samps, x_samples, ini_Y, jitter):
        ...
        y_samples, filtered_f = self._sample_Y(prng_states[1], ini_Y, f_samples)
...
class ModulatedRenewal(SparseGPFilterObservations):
    ...
    def __init__(self, gp, gp_mean, renewal, spikefilter=None):
        ...
        self.pp = PointProcess(
            gp.kernel.out_dims, renewal.dt, "log", ArrayTypes_[gp.array_type]
        )
    ...
    def variational_expectation(
        ...
        if log_predictive:
            Eq = jax.nn.logsumexp(
                vmap(vmap(llf), (None, 0, 0, 0))(
                    ys.T, log_rho_t, f_cov, prng_state
                ).mean(1)
                - jnp.log(num_samps),
                axis=0,
            )
    ...
    ### evaluation ###
    def evaluate_posterior_mean(self, prng_state, xs, taus, ys_filt, jitter):
        """
        Use the posterior mean to perform the time rescaling
        ...
        """
        if self.renewal.link_type == LinkTypes["log"]:
            log_modulator, log_rho_t_cov, _ = self._pre_rho_posterior(
                prng_state,
                xs[None, None],
                ys_filt,
                mean_only=False,
            ...
            log_modulator, log_rho_t_cov = (
                log_modulator[0, ..., 0],
                log_rho_t_cov[0, ..., 0],
            )  # (obs_dims, ts)

            log_hazard = vmap(self.renewal.log_hazard)(taus.T).T
            log_rho_t_mean = log_modulator + log_hazard
    ...
    ### sample ###
    def sample_log_conditional_intensity(
        self, prng_state, ini_tau, x_eval, y, y_filt, jitter
    ):
        """
        ...
        """
        pre_modulator, _ = self._pre_rho_sample(
            prng_state, x_eval, y_filt, False, False, jitter
        )  # (num_samps, obs_dims, ts)
    ...
    def sample_prior(
        self, prng_state, num_samps, x_samples, ini_spikes, ini_tau, jitter
    ):
        """
        Sample from the generative model
...
class RateRescaledRenewal(SparseGPFilterObservations):
    ...
    def _rate_rescale(self, spikes, pre_rates, ini_t_tilde, compute_ll, return_t_tilde):
        ...
        def step(carry, inputs):
            t_tilde, ll, valid_ts, inv_valid_ts = carry
            ...
            if compute_ll:
                ll += jnp.where(
                    ...
                    + self.renewal.log_density(t_tilde),
                    0.0,
                )
                valid_ts = jnp.where(t_tilde, valid_ts + 1.0, valid_ts)
            ...
            return (
                ...
                valid_ts,
                inv_valid_ts,
            ), t_tilde if return_t_tilde else None
        ...
        (_, log_lik, _, inv_valid_ts), t_tildes = lax.scan(
            step, init=init, xs=(pre_rates, spikes)
        )
        return log_lik, inv_valid_ts, t_tildes
    ...
    def variational_expectation(
        ...
        Compute variational expectation of likelihood and KL divergence
        """
        pre_rates, KL = self._pre_rho_marg_sample(
            prng_state, xs, ys_filt, compute_KL=compute_KL, jitter=jitter
        )
        #    self._pre_rho_sample(
        #    prng_state, xs, ys_filt, prior=False, compute_KL=compute_KL, jitter=jitter
        ...
        rrs = lambda f: self._rate_rescale(
            spikes, f.T, ini_t_tilde, compute_ll=True, return_t_tilde=False
        )[:2]
        ll, inv_valid_ts = vmap(rrs)(pre_rates)
        ll = jnp.nansum(
    ...
    def evaluate_posterior_mean(self, prng_state, xs, ys_filt, jitter):
        ...
        """
        if self.renewal.link_type == LinkTypes["log"]:
            log_rate_mean, log_rate_cov, _ = self._pre_rho_posterior(
                prng_state,
                xs[None, None],
                ys_filt,
                mean_only=False,
            ...
            log_rate_mean, log_rate_cov = (
                log_rate_mean[0, ..., 0],
                log_rate_cov[0, ..., 0],
            )  # (obs_dims, ts)

            post_rho_mean = jnp.exp(log_rate_mean + log_rate_cov / 2.0)
        else:
            raise NotImplementedError
    ...
    ### sample ###
    def sample_log_conditional_intensity(
        self, prng_state, ini_t_tilde, x_eval, y, y_filt, jitter
    ):
        """
        ...
        :param jnp.ndarray y: spike train corresponding to segment (neurons, ts),
                              y = y_filt[..., filter_length:] + last time step
        """
        f_samples, _ = self._pre_rho_sample(
            prng_state, x_eval, y_filt, False, False, jitter
        )  # (num_samps, obs_dims, ts)
        num_samps = pre_rates.shape[0]
        y = jnp.broadcast_to(y, (num_samps,) + y.shape[1:])
        ...
        _, tau_tilde = vmap(self._rate_rescale, (0, 0, 0, None, None), (None, 0))(
            spikes, rates, ini_t_tilde, False, True
        )  # (num_samps, ts, obs_dims)
    ...
    def sample_conditional_ISI(
        ...
        t_tilde_eval = rate * t_eval * self.renewal.mean_scale()[:, None]

        log_dens = vmap(vmap(self.renewal.log_density), 2, 2)(t_tilde_eval)
        return jnp.exp(log_dens)  # (num_samps, obs_dims, ts)

    def sample_prior(
        self, prng_state, num_samps, x_samples, ini_spikes, ini_t_tilde, jitter
    ):
        """
        Sample from the generative model
        Sample spike trains from the modulated renewal process.
...
class NonparametricPointProcess(Observations):
    ...
    def _refractory_mean(self, tau_tilde):
        ...
        return delta * (1.0 - tau_tilde) ** (div_taus) + self.mean_bias
    ...
    def _log_rho_tilde_post(self, tau_tilde, isi, x, mean_only, compute_KL, jitter):
        """
        Obtain the log conditional intensity given input path along which to evaluate
    ...
    def _sample_spikes(self, prng_state, timesteps, ini_tau, past_ISIs, x_eval, jitter):
        ...
        def step(carry, inputs):
            ...
            log_rho_tilde = self._log_rho_tilde_sample(
                prng_gp, tau_tilde[..., None], past_ISI, x, False, jitter
            )[..., 0]
    ...
    ### inference ###
    def _get_log_rho_post(self, jitter, xs, deltas, compute_KL):
        ...
        log_rho_tilde_mean, log_rho_t_cov, KL = self._log_rho_tilde_post(
            tau_tilde, isi, xs, mean_only=False, compute_KL=compute_KL, jitter=jitter
        )
        log_rho_t_mean = (
            log_rho_tilde_mean + log_dtilde_dt[None, ..., None]
    ...
    def variational_expectation(
        ...
        log_rho_t_mean, log_rho_t_cov, KL = self._get_log_rho_post(
            jitter, xs, deltas, compute_KL
        )

        log_rho_t_mean = log_rho_t_mean.transpose(
            0, 2, 1, 3
        )  # (num_samps, ts, out_dims, 1)
        log_rho_t_cov = log_rho_t_cov[..., 0].transpose(
            0, 2, 1
        )  # (num_samps, ts, out_dims, 1)
    ...
    ### evaluation ###
    def evaluate_log_conditional_intensity(self, xs, deltas, jitter):
        """
        ...
        log_rho_t_mean, log_rho_t_cov, _ = self._get_log_rho_post(
            jitter, xs, deltas, False
        )
        log_rho_t_mean = (
            log_rho_tilde_mean + log_dtilde_dt[None, ..., None]
        )  # (num_samps, out_dims, ts, 1)
    ...
    def evaluate_posterior_mean(self, xs, deltas, jitter):
        """
        Use the posterior mean to perform the time rescaling
        ...
        """
        log_rho_t_mean, log_rho_t_cov, _ = self._get_log_rho_post(
            jitter, xs[None, None], deltas, False
        )
        log_rho_t_mean, log_rho_t_cov = (
            log_rho_t_mean[0, ..., 0],
            log_rho_t_cov[0, ..., 0],
    ...
    def _sample_log_ISI_tilde(
        ...
        )  # vmap mc, then out_dims
        vvvinterp = vmap(
            vmap(vmap(jnp.interp, (0, None, None), 0)), (None, None, 0), 0
        )  # vmap mc, then out_dims, then evals
        ...
        isi_cond = (
            isi_cond[:, None].repeat(cat_pts, axis=1) if isi_cond is not None else None
        )
        x_cond = (
            jnp.broadcast_to(
        ...
        return log_rho_tilde - int_rho_tau_tilde - log_normalizer[..., None]
    ...
    def sample_conditional_ISI(
        ...
        log_ISI_tilde = self._sample_log_ISI_tilde(
            prng_state,
            num_samps,
            tau_tilde,
            isi_cond,
    ...
    def sample_conditional_ISI_expectation(
        ...
        log_ISI_tilde_pts = self._sample_log_ISI_tilde(
            prng_state,
            num_samps,
            tau_tilde_pts,
            isi_cond,
    ...
    def ELBO(self, prng_state, num_samps, jitter, tot_ts, data, lik_int_method):
        ...
        if type(self.obs_model) == ModulatedFactorized:
            ...
            Ell, KL_o = self.obs_model.variational_expectation(
                prng_key_o, jitter, xs, ys, ys_filt, True, lik_int_method
            )
            ...
            taus = deltas[..., 0]
            Ell, KL_o = self.obs_model.variational_expectation(
                prng_key_o, jitter, xs, taus, ys, ys_filt, True, lik_int_method
            )
            ...
            Ell, KL_o = self.obs_model.variational_expectation(
                prng_key_o, jitter, xs, deltas, ys, True, lik_int_method
            )
