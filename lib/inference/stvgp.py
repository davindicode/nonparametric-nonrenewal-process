...
from .base import FilterObservations, Observations
...
class NonparametricPointProcess(Observations):
    ...
    def _refractory_mean(self, tau):
        ...
        return (
            delta * (tau + 1.0) ** (-self.warp_tau / self.refract_tau) + self.mean_bias
        )
    ...
    def _log_rho_from_gp_sample(
        self, prng_state, num_samps, tau_eval, isi_eval, x_eval, prior, jitter
    ):
        """
    ...
    def _log_rho_from_gp_post(
        self, prng_state, tau_eval, isi_eval, x_eval, mean_only, compute_KL, jitter
    ):
        """
        Obtain the log conditional intensity given input path along which to evaluate
    ...
    def _sample_spikes(
        self, prng_state, timesteps, ini_t_since, past_ISIs, x_eval, jitter
    ):
        """
        ...
        def step(carry, inputs):
            ...
            # compute intensity
            log_rho_tau = vmap(
                self._log_rho_from_gp_sample,
                (
                    0,
                    None,
                    0,
                    None if past_ISI is None else 0,
                    None if x is None else 0,
                    None,
                    None,
                ),
                0,
            ...
            if past_ISI is not None:
                shift_ISIs = jnp.concatenate(
                    (t_since[..., None], past_ISI[..., 0, :-1]), axis=-1
                )
                past_ISI = jnp.where(
    ...
    def VI(self, prng_state, num_samps, jitter, xs, deltas, ys, lik_int_method):
        ...
        log_rho_tau_mean = log_rho_tau_mean.transpose(0, 2, 1)[
            ..., None
        ]  # (num_samps, ts, out_dims, 1)
        log_rho_tau_cov = log_rho_tau_cov.transpose(
            0, 2, 1
        )  # (num_samps, ts, out_dims, 1)
        log_rho_tau_cov = vmap(vmap(jnp.diag))(
            log_rho_tau_cov
        ...
        Ell = vmap(vmap(llf))(
            log_rho_tau_mean, log_rho_tau_cov
        ).mean()  # mean over mc and ts

        return Ell, KL.mean()

    ### evaluation ###
    def evaluate_log_conditional_intensity(
        self, prng_state, t_eval, isi_eval, x_eval, jitter
    ):
        """
        Evaluate the conditional intensity along an input path
        """
        tau_eval, log_dtau_dt_eval = vmap(
        ...
        log_rho_t_mean = (
            log_rho_tau_mean + log_dtau_dt_eval
    ...
    def sample_conditional_ISI(
        ...
        vvinterp_ = vmap(
            vmap(jnp.interp, (None, None, 0), 0), (None, None, 0), 0
        )  # vmap mc, then out_dims
        vvinterp = vmap(
            vmap(jnp.interp, (None, 0, 0), 0), (None, None, 0), 0
        )  # vmap mc, then out_dims
        vvvinterp = vmap(
            vmap(vmap(jnp.interp, (0, None, 0), 0), (None, None, 0), 0),
            (1, None, None),
            2,
        )  # vmap mc, then out_dims, then cubature
        vvquad_integrate = vmap(
            vmap(quad_integrate, (None, 0, None, None), (0, 0)),
            (None, 0, None, None),
            (0, 0),
