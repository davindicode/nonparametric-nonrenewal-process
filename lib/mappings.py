...
from jax import jit, jacrev, random, vmap, value_and_grad, grad, tree_map, vjp
...
class Mapping(object):
    ...
    def sample_prior(self, params, prng_state, x, jitter):
        """
        """
        raise NotImplementedError('Variational posterior for this mapping is not implemented')
...
class Constant(Mapping):
    ...
    def sample_posterior(self, params, var_params, prng_state, x, jitter, compute_KL):
        ...
        :return:
            sample of shape (time, num_samps, f_dims)
        """
        samples, _, KL = self.evaluate_posterior(x, params, var_params, True, compute_KL, jitter)
        return samples, KL
...
class tSVGP(Mapping):
    ...
    def __init__(self, kernel, mean_f, induc_locs, var_params, RFF_num_feats=0):
        ...
        if (not 'lambda_1' in var_params.keys()) or (not 'chol_Lambda_2' in var_params.keys()):
            raise ValueError('Missing keys in site parameter dictionary')
        
        if induc_locs.shape[2] != kernel.in_dims:
            raise ValueError('Dimensions of inducing locations do not match kernel input dimensions')
        if induc_locs.shape[0] != kernel.out_dims:
            raise ValueError('Dimensions of inducing locations do not match kernel output dimensions')
    ...
    def evaluate_posterior(self, x, params, var_params, mean_only, compute_KL, compute_aux, jitter):
        """
        :param jnp.array x: input of shape (time, num_samps, x_dims, 1)
        :returns:
        ...
        Kxx = vmap(K, (2, 2), 1)(x[None, ...], x[None, ...])  # (f_dims, num_samps, time, time)
        Kxx = jnp.broadcast_to(Kxx, (self.f_dims, num_samps, ts, ts))
        
        Kzx = vmap(K, (None, 2), 1)(z, x[None, ...])  # (f_dims, num_samps, num_induc, time)
        ...
        post_means = Kxz @ Rinv_lambda_1[:, None, ...].repeat(num_samps, axis=1) + \
            mean_f[:, None, None, None]  # (f_dims, num_samps, time, 1)
        
        if mean_only is False:
            invR_Kzx = cho_solve(
                (chol_R[:, None, ...].repeat(num_samps, axis=1), True), Kzx) # (f_dims, num_samps, num_induc, time)
            post_covs = Kxx - Kxz_invKzz @ Kzx + Kxz @ invR_Kzx # (f_dims, num_samps, time, time)
        else:
            post_covs = None
        
        if compute_KL:
            trace_term = jnp.trace(cho_solve((chol_R, True), Kzz)).sum()
            quadratic_form = (Rinv_lambda_1.transpose(0, 2, 1) @ Kzz @ Rinv_lambda_1).sum()
            log_determinants = (jnp.log(vdiag(chol_R)) - jnp.log(vdiag(chol_Kzz))).sum()
            KL = .5 * (trace_term + quadratic_form - self.num_induc) + log_determinants
        else:
            KL = 0.
    ...
    def sample_prior(self, params, prng_state, x, jitter):
        ...
        if self.RFF_num_feats > 0:  # random Fourier features
            ...
            ks = self.kernel.sample_spectrum(
                kern_hyp, prng_keys[0], num_samps, self.RFF_num_feats)  # (num_samps, f_dims, feats, x_dims)            
            phi = 2*jnp.pi * jr.uniform(
                prng_keys[1], shape=(num_samps, self.f_dims, self.RFF_num_feats))

            samples = mean_f[None, None, :] + jnp.sqrt(2. * variance / self.RFF_num_feats) * \
                (jnp.cos((ks[None, ...] * x[..., None, :]).sum(-1) + phi)).sum(-1)  # (time, num_samps, f_dims)
            
        else:
            K = lambda x: self.kernel.K(x, x, kern_hyp)
            Kxx = vmap(K, 1, 1)(x.transpose(2, 1, 0, 3))  # (f_dims, num_samps, time, time)
            eps_I = jnp.broadcast_to(jitter*jnp.eye(ts), 
                                     (self.f_dims, 1, ts, ts))
            cov = Kxx + eps_I
            mean = jnp.broadcast_to(mean_f[:, None, None, None], (self.f_dims, num_samps, ts, 1))
            samples = sample_gaussian_noise(prng_state, mean, cov)[..., 0].transpose(2, 1, 0)
        ...
        if self.RFF_num_feats > 0:
            ...
            x_aug = jnp.concatenate(
                (x[..., None, :].repeat(self.f_dims, axis=2), 
                 z[:, None, ...].repeat(num_samps, axis=1)), axis=0
            ...
            noise = solve_triangular(
                ...
                jr.normal(prng_keys[1], shape=(self.f_dims, num_samps, self.num_induc, 1)), 
                lower=True
            )
