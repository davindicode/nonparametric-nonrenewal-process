...
class Hodgkin_Huxley:
    ...
    def euler_int(self, T, runs, I_ext, ic, dt=0.001, prin=1000):
        ...
        for k in range(runs):
            state[k, 0, :] = ic[
                k, :
            ]  # [-6.49997224e+01, 5.29342176e-02, 5.96111046e-01, 3.17681168e-01]
        ...
        for t in iterator:
            ...
            ds[:, 1] = (
                alpha_m(state[:, t, 0]) * (1 - state[:, t, 1])
                - beta_m(state[:, t, 0]) * state[:, t, 1]
            )
            ds[:, 2] = (
                alpha_h(state[:, t, 0]) * (1 - state[:, t, 2])
                - beta_h(state[:, t, 0]) * state[:, t, 2]
            )
            ds[:, 3] = (
                alpha_n(state[:, t, 0]) * (1 - state[:, t, 3])
                - beta_n(state[:, t, 0]) * state[:, t, 3]
            )
    ...
    def euler_int(self, T, runs, I_ext, ic, dt=0.001, prin=1000):
        ...
        for t in iterator:
            ds[:, 0] = (
                1
                / self.tau_u
                * (state[:, t, 0] - state[:, t, 0] ** 3 / 3.0 - state[:, t, 1] + I_ext)
            )
            ds[:, 1] = (
                1 / self.tau_w * (self.b_0 + self.b_1 * state[:, t, 0] - state[:, t, 1])
            )
...
class Izhikevich:
    ...
    def euler_int(self, T, runs, I_ext, ic, dt=0.1, prin=1000):
        ...
        for t in iterator:
            ...
            reset = state[:, t, 0] >= 30.0
            if reset.sum() > 0:
                reset_state[:, 1] = state[:, t, 1] + self.d
                state[:, t + 1] = reset[:, None] * reset_state + (1 - reset)[
                    :, None
                ] * (state[:, t] + ds * dt)
