...
def LWL_model(p, sample_bin, behav_tuple, spiketrain, kernel_func):
    ...
    p_t = np.concatenate((x_t[:, None], y_t[:, None]), axis=1)[None, ...] # pos, timestep, dim
    ...
    for trn in spiketrain:
        ...
        x_sum = (x*(kern*trn[None, spiketimes])).sum(-1)/(kern*trn[None, spiketimes]).sum(-1)
...
def phase_field(sample_bin, covariates, spat_bins, spiketimes):
    ...
    for times in spiketimes:
        ...
        if dim == 1:
            ...
            a = [[[[] for l in range(bin_count[2])] for k in range(bin_count[1])] for r in range(bin_count[0])]
        else:
...
def fit_oscillatory_ISI(sample_bin, ISI, bins, lr=1e-3, max_iters=1000, loss_tolerance=0, 
                        tolerance_steps=10, dev='cpu'):
    """
    ...
    omega = Parameter(torch.empty((units, 1), device=dev).fill_(1./(500.0*sample_bin)))
    ...
    for k in iterator:
        optimizer.zero_grad()
        acg_func = amp * ((x_tens+1e-12)/tau)**(alpha-1) * torch.exp(-x_tens/tau) * \
...
def EEG_fit(sample_bin, eeg_t, lag_range=1250, time_window=1000000, lr=1e-3, max_iters=1000, 
            loss_tolerance=0, tolerance_steps=10, dev='cpu'):
    r"""
    ...
    optimizer = optim.Adam([omega] + [phi] + [amp] + [tau] + [power] + [toffs] + [offs], lr=lr)
    ...
    for k in iterator:
        optimizer.zero_grad()
        acg_func = amp * (lag_tens/tau+toffs)**(power) * torch.cos(omega * lag_tens + phi) + offs
