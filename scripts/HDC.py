...
def init_argparse():
    parser = argparse.ArgumentParser(
        usage="%(prog)s [OPTION] [FILE]...", description="Run diode simulations."
    )
    parser.add_argument(
...
### data
def get_dataset(session_id, phase):
    data = np.load("../data/Mouse{}_{}_ext.npz".format(session_id, phase))
...
def inputs_used(x_mode, z_mode, cov_tuple, batch_info, tensor_type):
    ...
    hd_t, w_t, s_t, x_t, y_t, time_t, isi1, isi2, isi3, isi4, isi5 = b
    ...
    elif x_mode[:-1] == "hd-w-s-pos-t-isi":
        H_isi = int(x_mode[-1])
...
def enc_used(
    ...
    def kernel_used(x_mode, z_mode, cov_tuple, num_induc, out_dims, var, tensor_type):
        ...
        """
        hd_t, w_t, s_t, x_t, y_t, time_t, isi1, isi2, isi3, isi4, isi5 = cov_tuple
        ...
        if x_mode[:-1] == "hd-isi":
            ...
            kernel_tuples = [("variance", v), ("SE", "torus", torch.tensor([l_ang]))]
            ...
            kernel_tuples = [
                ...
                ("SE", "euclid", torch.tensor([l_ISI] * H_isi)),
            ]
    ...
    if map_mode[:4] == "svgp":
        ...
        kernelobj, inducing_points = kernel_used(
            x_mode, z_mode, cov_tuple, num_induc, inner_dims, var, tensor_type
        )
...
def main():
    ...
    dataset_tuple = get_dataset(session_id, phase)
