...
def CA_dataset(dataset, bin_size):
    ...
    tbin, resamples, rc_t, (rx_t, ry_t, rtheta_t, rhd_t) = utils.neural.bin_data(
        ...
        spktrain,
        track_samples,
        (x_t, y_t, np.unwrap(theta_t), np.unwrap(hd_t)),
        average_behav=True,
        binned=True,
    )
...
def main():
    ...
    phase = "wake"
    rcov, neurons, tbin, resamples, rc_t, max_count, metainfo, model_name = HDC_dataset(
        session_id, phase, 1
    )
    ...
    # load CA dataset
    dataset = "maze15_half-"
    rcov, neurons, tbin, resamples, rc_t, max_count, metainfo, model_name = CA_dataset(
        dataset, 1
    )
    ...
    for data in datasets:
        ...
        cov = lib.bpp.pp_covariates(
            rcov, sep_t_spike, max_Horder, hist_len, tbin
        ).numpy()
