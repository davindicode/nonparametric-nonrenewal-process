...
def plot_tuning_and_ISIs(fig, tuning_dict, name, cs):
    ...
    for n in range(3):
        for m in range(3):
            ...
            ax.imshow(pos_est_rate[k], vmin=0.0, origin="lower", cmap="viridis")
            lib.utils.plots.decorate_ax(ax)
            for spine in ax.spines.values():
                spine.set_edgecolor(cs[k])
                spine.set_linewidth(2)
    ...
    for en, ne in enumerate(ISI_neuron_conds):
        ...
        ax.plot(
            ISI_t_eval, gt_dens, c=cs[ne], alpha=1.0, label="truth" if en == 1 else None
        )
        ax.plot(
            ISI_t_eval,
            dens.T,
    ...
    ax.annotate(
        ...
        arrowprops=dict(
            arrowstyle="simple, head_width=0.7, head_length=1.0, tail_width=0.1",
            color="gray",
            alpha=0.3,
        ),
...
def plot_kernel_lens(fig, data_dict, cs):
    ...
    for n in range(9):
        ...
        tlen = len_tau[n]
        ax.scatter(
            n, tlen, marker=markers[0], c=cs[n], s=20, label="lag 0" if n == 0 else None
        )
        for k in range(1, 4):
            ax.scatter(
                n,
        ...
        ax.plot([-0.5, 9.5], 3.0 * np.ones(2), "--", color="gray")
        ax.set_xlim([-0.5, 9.5])
    ...
    for en, n in enumerate(use_reg_config_names):
        ...
        for n in range(len(sort_cdfs)):
            if sort_cdfs[n] is not None:
                ax.plot(
                    sort_cdfs[n],
                    np.linspace(0.0, 1.0, len(sort_cdfs[n])),
                    c=cs[n],
                    alpha=0.8,
                )
